...
// Encode the original data with the RSA private key
    ...
    String   encryptedString=Base64.encodeToString(rsaencodedBytes, Base64.DEFAULT);
...
public static String dititaldecryptnencrypt(byte[] encryptedMessageHash,byte[] input,int mode ) throws Exception{
  ...
  //  byte[] encryptedMessageHash = signature;//Files.readAllBytes(Paths.get("digital_signature_1"));
...
return key;
    ...
    //rc2 not working because library is not found we can include jar file and it is working
    //ALl engines of crypto are available :https://github.com/bcgit/bc-java/tree/master/core/src/main/java/org/bouncycastle/crypto/engines
    //
    /*
    public static String encryptrc2(byte[] input,int mode, String pass) throws Exception {
        ...
        key = generateKeyAes(pass);
        SecretKeySpec secretKey = new SecretKeySpec(key, new RC2Engine().getAlgorithmName());
